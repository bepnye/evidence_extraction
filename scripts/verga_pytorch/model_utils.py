...
def label_to_val(label):
	...
	return {'INCR': 2, 'DECR': 3, 'SAME': 1, 'NULL': 0, 'Null': 0, 'CID': 1}.get(label)
...
def generate_ner_labels(text, mapping, relations, label_config):
	...
	for i in range(len(text)): 
		if i in intervention_idx: 
			ner_labels.append(label_config['NER']['E1'])
		elif i in outcome_idx: 
			ner_labels.append(label_config['NER']['E2'])
		else:
			ner_labels.append(label_config['NER']['NULL'])

	return ner_labels

def print_relation(rel):
	e1, e2 = rel
	print('E1: {}'.format(e1.text))
	print('\n\t'.join(['Mentions: '] + [str(m) for m in e1.mentions]))
	print()
	print('E2: {}'.format(e2.text))
	print('\n\t'.join(['Mentions: '] + [str(m) for m in e2.mentions]))
