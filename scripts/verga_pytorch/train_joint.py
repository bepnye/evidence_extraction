...
def create_entities_from_offsets(text, groups, offsets):
	...
	for grp, span in zip(groups, offsets):
		selected_tokens = text[span[0]:span[1]] # this is tokenized... sorry :(
		selected_text = ' '.join(TOKENIZER.convert_ids_to_tokens(selected_tokens))
...
def create_simulated_relations(inputs, intv_groups, intv_offsets, out_groups, out_offsets, label_config, test_set = False):
	...
	for intv, out in pairs:
		...
		if not(success) and (random.uniform(0, 1) < 0.01 or test_set):
			batch_labels.append(label_config['NER']['NULL']) # add null
			batch_input['labels'].append(label_config['NER']['NULL'])
			batch_input['relations'].append((intv, out))
...
def agglomerative_coref(inputs, ner_scores, bert_embedds, true_labels, label_config, test_set = False): 
	...
	for i in range(len(arg_scores)):
		intervention_embeds, intv_offsets = collapse_mentions([bert_embedds[i][idx] if sc == label_config['NER']['E1'] else None for idx, sc in enumerate(arg_scores[i])])
		outcome_embeds, outcome_offsets = collapse_mentions([bert_embedds[i][idx] if sc == label_config['NER']['E2'] else None for idx, sc in enumerate(arg_scores[i])])	  
...
def evaluate_model(relation_model, ner_model, label_config, criterion, test, epoch, batch_size, teacher_forcing=False, dump = None):
	...
	for batch_range in range(0, len(test), batch_size):
		...
		ner_batch_labels = PaddedSequence.autopad([lab[:CUT_OFF] for lab in ner_labels], batch_first=True, padding_value=label_config['NER']['NULL'], device='cuda')
		...
		with torch.no_grad():
			...
			if not(teacher_forcing):
				...
				scores = soft_scoring(inputs, orig_inputs, relation_outputs.argmax(dim=-1), label_config['REL']['NULL'])
...
def train_model(ner_model, relation_model, df, parameters):
	...
	ner_loss_weight     = parameters.ner_loss_weight
	rel_loss_weight     = 1 - ner_loss_weight
	...
	assert(0.0 <= ner_loss_weight<= 1.0)

	path_ner = "ner_model_lr_{}_epochs_{}_ner_loss_w_{}_start_tfr_{}_decay_{}.pth".format(learning_rate, epochs, ner_loss_weight, teacher_force_ratio, teacher_force_decay)
	path_relation = "relation_model_lr_{}_epochs_{}_ner_loss_w_{}_start_tfr_{}_decay_{}.pth".format(learning_rate, epochs, ner_loss_weight, teacher_force_ratio, teacher_force_decay)		   

	with open('loss.csv', 'a+') as loss_record:
		loss_record.write('batch_size: {} | ner_weight: {} | tf_ratio: {} | tf_decay: {}\n'.format(\
				batch_size, ner_loss_weight, teacher_force_ratio, teacher_force_decay))
	...
	for epoch in range(epochs):
		...
		for batch_range in range(0, len(train_data), batch_size):
			...
			ner_batch_labels = PaddedSequence.autopad([lab[:CUT_OFF] for lab in ner_labels[batch_range: batch_range + batch_size]], batch_first=True, padding_value=label_config['NER']['NULL'], device='cuda')
			text_inputs = [torch.tensor(input_['text'][:CUT_OFF]).cuda() for input_ in inputs]	
			padded_text = PaddedSequence.autopad(text_inputs, batch_first = True, padding_value=0, device='cuda')
			...
			ner_loss, ner_scores, hidden_states = ner_model(padded_text.data,
					attention_mask=ner_mask, labels = ner_batch_labels.data)
			
			batch_ner_true = ner_batch_labels.data.reshape(-1).cpu().tolist()
			batch_ner_pred = torch.argmax(ner_scores, dim = -1).reshape(-1).cpu().tolist()
			epoch_ner_true += batch_ner_true
			epoch_ner_pred += batch_ner_pred
			...
			rel_loss = criterion(relation_outputs, torch.tensor(labels).cuda())

			sum_loss = (ner_loss_weight * ner_loss) + (rel_loss_weight * rel_loss)
			sum_loss.backward()
			optimizer.step()

			batch_ner_losses.append(ner_loss.item())
			batch_rel_losses.append(rel_loss.item())
			batch_sum_losses.append(sum_loss.item())

			# next labels
			label_offset += len(relation_outputs)

		epoch_ner_loss = np.mean(batch_ner_losses)
		epoch_rel_loss = np.mean(batch_rel_losses)
		epoch_sum_loss = np.mean(batch_sum_losses)

		print("Training NER report ({:.3f} loss):\n{}".format(epoch_ner_loss, classification_report(epoch_ner_true, epoch_ner_pred)))
		with open('loss.csv', 'a+') as loss_record:
			loss_record.write('epoch: {} | rel_loss {:.5f} | ner_loss {:.5f} | ratio: {:.2f}\n'.format(\
					epoch, epoch_rel_loss, epoch_ner_loss, epoch_rel_loss/epoch_ner_loss))
		...
		print("Epoch {} Training Loss: {}\n".format(epoch, epoch_sum_loss))
		print("Dev results with teacher forcing")
		f1_score = evaluate_model(relation_model, ner_model, label_config, criterion, dev, epoch, batch_size, teacher_forcing = True, dump='dev_without_teacher_forcing.out')
...
def main(args=sys.argv[1:]):
	...
	parser.add_argument("--percent_train", dest="percent_train", type=float, default=1.0, help="What percent if the training data should we use?")
