...
class NerProcessor(object):
    ...
    def get_labels(self):
        ...
        return [NO_LABEL, "p", "i", "o", "[CLS]", "[SEP]"]

    def _create_example(self, lines, set_type):
        examples = []
        for (i, line) in enumerate(lines): # line = (w, p, c, l)
            guid  = "%s-%s" % (set_type, i)
            if FLAGS.label_idx == 0:
                label = tokenization.convert_to_unicode(line[0])
                text  = tokenization.convert_to_unicode(line[1])
            elif FLAGS.label_idx == 1:
                text  = tokenization.convert_to_unicode(line[0])
                label = tokenization.convert_to_unicode(line[1])
            else:
                raise Exception('Cannot predict label / token order in input file')
    ...
    def _read_data(cls, input_file):
        ...
        with codecs.open(input_file, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                contents = line.strip()
                if contents.startswith("-DOCSTART-") or contents.startswith('#<'):
                    continue
            ...
            for predict_example, prediction in zip(predict_examples, result):
                ...
                if FLAGS.collapse_wp:
                  ...
                  for wp_idx, wp_idx_group in wp_idx_groups:
                    ...
                    base_label = predict_example.labels[wp_idx]

                    _, tokens, labels, preds = zip(*wp_idx_group)
                    pred_id = collections.Counter(preds).most_common(1)[0][0]
                    pred_label = id2label[pred_id]

                    output_line = ' '.join([base_label, pred_label, base_token])
                    writer.write(output_line + '\n')

                else:
                  for token, label, pred_id in zip(tokenized_tokens, tokenized_labels, tokenized_preds):
                    output_line = ' '.join([label, id2label[pred_id], token])
                    writer.write(output_line + '\n')
